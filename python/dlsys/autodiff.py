...
class AddByConstOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        """TODO: Your code here"""
        # shape does not change here, so returning the shape of input
...
class MatMulOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        shapeA = input_shapes[0] if not node.matmul_attr_trans_A else (input_shapes[0][1], input_shapes[0][0])
        shapeB = input_shapes[1] if not node.matmul_attr_trans_B else (input_shapes[1][1], input_shapes[1][0])
        assert shapeA[1] == shapeB[0], "MatMul shape mismatch: {} cannot be multiplied with {}".format(shapeA, shapeB)
...
class ReduceSumAxisZeroOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        ...
        """
        """TODO: Your code here"""
        #summing along axis 0 removes the first dimension of the shape
...
class BroadcastToOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        """TODO: Your code here"""
        #output shape matches the second node's shape, as the first node is broadcasted to this shape
...
class Executor(object):
    ...
    def infer_shape(self, feed_shapes):
        ...
        self.node_to_shape_map = feed_shapes.copy()
        ...
        # Debugging: Print out the nodes and their inferred shapes
        for node, shape in self.node_to_shape_map.items():
            print("Node: {}, Inferred shape: {}".format(node, shape))
    ...
    def memory_plan(self, feed_shapes):
        ...
        for node in self.topo_order:
            if node not in feed_shapes:
                ...
                # For feed_dict nodes, just use the provided shape
                self.node_to_arr_map[node] = ndarray.array(feed_shapes[node], ctx=self.ctx)

        # Debugging: Print out the nodes and their allocated shapes
        for node, arr in self.node_to_arr_map.items():
            print("Node: {}, Shape: {}".format(node, arr.shape))
